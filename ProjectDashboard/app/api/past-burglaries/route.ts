...
export async function GET() {
  try {
    ...
    const actualDataPath = path.join(process.cwd(), '../../outputs/actual_past_year_500m.csv');
    ...
    const mockData = {
      features: {
        ...
        features: Array.from({ length: 50 }, (_, i) => ({
          ...
          properties: {
            cell_id: i,
            actual_past_year: Math.floor(Math.random() * 30),
            area_km2: 0.25
          }
        }))
      },
      maxValue: 30,
      timeLabel: "Jan 2023 - Dec 2023",
      wardBoundaries: {
        type: "FeatureCollection",
    ...
    console.error('Error fetching past burglaries data:', error);
    return NextResponse.json(
      { error: 'Failed to fetch past burglaries data' },
      { status: 500 }
    );
  }
} 